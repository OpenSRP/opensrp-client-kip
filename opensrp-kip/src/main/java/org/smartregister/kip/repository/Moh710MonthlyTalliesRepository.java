...
import org.smartregister.kip.domain.Moh710MonthlyTally;
...
public class Moh710MonthlyTalliesRepository extends BaseRepository {
    ...
    public Moh710MonthlyTalliesRepository() {
        super();
    }
    ...
    public List<Date> findUneditedDraftMonths(Date startDate, Date endDate) {
        List<String> allTallyMonths = KipApplication.getInstance().moh710DailyTalliesRepository()
     ...
     * @return
     */
    public List<Moh710MonthlyTally> findDrafts(String month) {
        Cursor cursor = null;
        List<Moh710MonthlyTally> monthlyTallies = new ArrayList<>();
        try {

            Log.w(TAG, "Using daily tallies instead of monthly");
            Map<Long, List<Moh710DailyTally>> dailyTallies = KipApplication.getInstance().moh710DailyTalliesRepository().findTalliesInMonth(DF_YYYYMM.parse(month));
            for (List<Moh710DailyTally> curList : dailyTallies.values()) {
                Moh710MonthlyTally curTally = addUpDailyTallies(curList);
                if (curTally != null) {
     ...
     * @return
     */
    public List<Moh710MonthlyTally> findDrafts(Date startDate, Date endDate) {
        List<Moh710MonthlyTally> monthlyTallies = new ArrayList<>();
        try {

            Log.w(TAG, "Using daily tallies instead of monthly");
            Map<Long, List<Moh710DailyTally>> dailyTallies = KipApplication.getInstance().moh710DailyTalliesRepository().findTallies(startDate, endDate);
            for (List<Moh710DailyTally> curList : dailyTallies.values()) {
                Moh710MonthlyTally curTally = addUpDailyTallies(curList);
                if (curTally != null) {
                    monthlyTallies.add(curTally);
     ...
     * @return
     */
    public List<Moh710MonthlyTally> find(String month) {
    ...
    private Moh710MonthlyTally addUpDailyTallies(List<Moh710DailyTally> dailyTallies) {
        String userName = KipApplication.getInstance().context().allSharedPreferences().fetchRegisteredANM();
        Moh710MonthlyTally monthlyTally = null;
        double value = 0d;
        for (int i = 0; i < dailyTallies.size(); i++) {
            if (i == 0) {
                monthlyTally = new Moh710MonthlyTally();
    ...
    private List<Moh710MonthlyTally> readAllDataElements(Cursor cursor) {
        List<Moh710MonthlyTally> tallies = new ArrayList<>();
        HashMap<Long, MohIndicator> indicatorMap = KipApplication.getInstance().moh710IndicatorsRepository().findAll();

        try {
            if (cursor != null && cursor.getCount() > 0) {
                for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                    Moh710MonthlyTally curTally = extractMonthlyTally(indicatorMap, cursor);
                    if (curTally != null) {
    ...
    private Moh710MonthlyTally extractMonthlyTally(HashMap<Long, MohIndicator> indicatorMap, Cursor cursor) throws ParseException {
        long indicatorId = cursor.getLong(cursor.getColumnIndex(COLUMN_INDICATOR_ID));
        if (indicatorMap.containsKey(indicatorId)) {
            Moh710MonthlyTally curTally = new Moh710MonthlyTally();
     ...
     */
    public List<Moh710MonthlyTally> findEditedDraftMonths(Date startDate, Date endDate) {
        Cursor cursor = null;
        List<Moh710MonthlyTally> tallies = new ArrayList<>();
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                    ...
                    Moh710MonthlyTally tally = new Moh710MonthlyTally();
