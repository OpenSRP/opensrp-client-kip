...
import org.smartregister.kip.wrapper.VaccineViewRecordUpdateWrapper;
import org.smartregister.kip.wrapper.WeightViewRecordUpdateWrapper;
...
public class ChildSmartClientsProvider implements SmartRegisterCLientsProviderForCursorAdapter {
    ...
    public void getView(Cursor cursor, SmartRegisterClient client, final View convertView) {
        ...
        fillValue((TextView) convertView.findViewById(R.id.child_zeir_id), getValue(pc.getColumnmaps(), KipConstants.KEY.ZEIR_ID, false));

        String firstName = getValue(pc.getColumnmaps(), KipConstants.KEY.FIRST_NAME, true);
        String lastName = getValue(pc.getColumnmaps(), KipConstants.KEY.LAST_NAME, true);
        String childName = getName(firstName, lastName);

        String motherFirstName = getValue(pc.getColumnmaps(), KipConstants.KEY.MOTHER_FIRST_NAME, true);
        if (StringUtils.isBlank(childName) && StringUtils.isNotBlank(motherFirstName)) {
            childName = "B/o " + motherFirstName.trim();
        }
        fillValue((TextView) convertView.findViewById(R.id.child_name), childName);

        String motherName = getValue(pc.getColumnmaps(), KipConstants.KEY.MOTHER_FIRST_NAME, true) + " " + getValue(pc, KipConstants.KEY.MOTHER_LAST_NAME, true);
        ...
        String dobString = getValue(pc.getColumnmaps(), KipConstants.KEY.DOB, false);
        ...
        fillValue((TextView) convertView.findViewById(R.id.child_card_number), pc.getColumnmaps(), KipConstants.KEY.EPI_CARD_NUMBER, false);

        String gender = getValue(pc.getColumnmaps(), KipConstants.KEY.GENDER, true);
        ...
        String lostToFollowUp = getValue(pc.getColumnmaps(), KipConstants.KEY.LOST_TO_FOLLOW_UP, false);
        String inactive = getValue(pc.getColumnmaps(), KipConstants.KEY.INACTIVE, false);
    ...
    private void updateRecordVaccination(VaccineViewRecordUpdateWrapper updateWrapper) {
        ...
        List<Map<String, Object>> sch = generateScheduleList(KipConstants.KEY.CHILD, new DateTime(updateWrapper.getDobString()), recievedVaccines, updateWrapper.getAlertList());
        ...
        if (nv != null) {
            DateTime dueDate = (DateTime) nv.get(KipConstants.KEY.DATE);
            VaccineRepo.Vaccine vaccine = (VaccineRepo.Vaccine) nv.get(KipConstants.KEY.VACCINE);
            stateKey = VaccinateActionUtils.stateKey(vaccine);
            if (nv.get(KipConstants.KEY.ALERT) == null) {
                state = State.NO_ALERT;
            } else if (((Alert) nv.get(KipConstants.KEY.ALERT)).status().value().equalsIgnoreCase(KipConstants.KEY.NORMAL)) {
                state = State.DUE;
            } else if (((Alert) nv.get(KipConstants.KEY.ALERT)).status().value().equalsIgnoreCase(KipConstants.KEY.UPCOMING)) {
            ...
            } else if (((Alert) nv.get(KipConstants.KEY.ALERT)).status().value().equalsIgnoreCase(KipConstants.KEY.URGENT)) {
                state = State.OVERDUE;
            } else if (((Alert) nv.get(KipConstants.KEY.ALERT)).status().value().equalsIgnoreCase(KipConstants.KEY.EXPIRED)) {
                state = State.EXPIRED;
            }
        } else {
            state = State.WAITING;
        ...
        if (state.equals(State.FULLY_IMMUNIZED)) {
            ...
            if (StringUtils.isNotBlank(stateKey) && (StringUtils.containsIgnoreCase(stateKey, KipConstants.KEY.WEEK) || StringUtils.containsIgnoreCase(stateKey, KipConstants.KEY.MONTH)) && !updateWrapper.getVaccines().isEmpty()) {
                Vaccine vaccine = updateWrapper.getVaccines().isEmpty() ? null : updateWrapper.getVaccines().get(updateWrapper.getVaccines().size() - 1);
                String previousStateKey = VaccinateActionUtils.previousStateKey(KipConstants.KEY.CHILD, vaccine);
        ...
        if (commonRepository != null) {
            ...
            if (commonPersonObject == null) { //Out of area -- doesn't exist in local database
                ...
                if (isWeightRecord) {
                    ...
                    String zeirId = getValue(pc.getColumnmaps(), KipConstants.KEY.ZEIR_ID, false);
                    ...
                    String motherBaseEntityId = getValue(pc.getColumnmaps(), KipConstants.KEY.MOTHER_BASE_ENTITY_ID, false);
    ...
    private class VaccinationAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            alerts = alertService.findByEntityIdAndAlertNames(entityId, VaccinateActionUtils.allAlertNames(KipConstants.KEY.CHILD));
            return null;
        }
