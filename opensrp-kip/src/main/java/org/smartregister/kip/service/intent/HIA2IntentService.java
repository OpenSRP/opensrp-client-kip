...
public class HIA2IntentService extends IntentService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        eventClientRepository = KipApplication.getInstance().eventClientRepository();
    ...
    private void generateDailyHia2Indicators() {
        try {
            ...
            String lastProcessedDate = KipApplication.getInstance().context().allSharedPreferences().getPreference(HIA2Service.HIA2_LAST_PROCESSED_DATE);
            ...
            for (Map<String, String> dates : reportDates) {
                ...
                KipApplication.getInstance().context().allSharedPreferences().savePreference(HIA2Service.HIA2_LAST_PROCESSED_DATE, updatedAt);
            }
        } catch (Exception e) {
            Log.e(TAG, e.getMessage(), e);
        }
    ...
    private Long countDaysAfterDueDate(Vaccine vaccine) {
        ...
        CommonRepository commonRepository = KipApplication.getInstance().context().commonrepository(KipConstants.CHILD_TABLE_NAME);
