    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            if (uniqueIdRepo.countUnUsedIds() == 0) { // first time pull no ids at all
                numberToGenerate = KipConstants.OPENMRS_UNIQUE_ID_INITIAL_BATCH_SIZE;
            } else if (uniqueIdRepo.countUnUsedIds() <= 250) { //maintain a minimum of 250 else skip this pull
                numberToGenerate = KipConstants.OPENMRS_UNIQUE_ID_BATCH_SIZE;
            } else {
                return;
            }

            String userName = KipApplication.getInstance().context().allSharedPreferences().fetchRegisteredANM();
            String password = KipApplication.getInstance().context().allSettings().fetchANMPassword();

            String localUrlString = KipConstants.openmrsUrl() + KipConstants.OPENMRS_IDGEN_URL + "?source=" + KipConstants.OPENMRS_UNIQUE_ID_SOURCE + "&numberToGenerate=" + numberToGenerate + "&username=" + userName + "&password=" + password;
//           // Convert the incoming data string to a URL.
            ...
            if (localURLConnection instanceof HttpURLConnection) {
                ...
                localHttpURLConnection.setRequestProperty("User-Agent", FileUtilities.getUserAgent(KipApplication.getInstance().context().applicationContext()));
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        uniqueIdRepo = KipApplication.getInstance().uniqueIdRepository();
