...
import org.smartregister.kip.tabfragments.ChildUnderFiveFragment;
import org.smartregister.kip.toolbar.ChildDetailsToolbar;
    ...
    private String getmetaDataForEditForm() {
        ...
        try {
            ...
            if (form != null) {
                ...
                Intent intent = new Intent(getApplicationContext(), KipJsonFormActivity.class);
...
//        tabLayout.setSelectedTabIndicatorColor(getResources().getColor(normalShade));
    ...
    public void onUndoVaccination(VaccineWrapper tag, View view) {
        if (tag != null && tag.getDbKey() != null) {
            final VaccineRepository vaccineRepository = KipApplication.getInstance().vaccineRepository();
...
//            setLastModified(true);
    ...
    public void updateClientAttribute(String attributeName, Object attributeValue) {
        try {
            ...
            int id = db.getWritableDatabase().update(KipConstants.CHILD_TABLE_NAME, contentValues, "base_entity_id" + "=?", new String[]{childDetails.entityId()});
            ...
            KipClientProcessor.getInstance(this).processClient(ecUpdater.getEvents(lastSyncDate, BaseRepository.TYPE_Unsynced));
    ...
    public KipApplication getVaccinatorApplicationInstance() {
        return KipApplication.getInstance();
    }
