...
import org.smartregister.kip.interactors.KipJsonFormInteractor;
import org.smartregister.kip.provider.MotherLookUpSmartClientsProvider;
import org.smartregister.kip.viewstates.KipJsonFormFragmentViewState;
...
public class KipJsonFormFragment extends JsonFormFragment {
    ...
    @Override
    protected KipJsonFormFragmentViewState createViewState() {
        return new KipJsonFormFragmentViewState();
    }

    @Override
    protected JsonFormFragmentPresenter createPresenter() {
        return new JsonFormFragmentPresenter(this, KipJsonFormInteractor.getInstance());
    }
    ...
    private void updateResults(final HashMap<CommonPersonObject, List<CommonPersonObject>> map) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity(), R.style.KipDialog);
        ...
        if (item.getItemId() == com.vijay.jsonwizard.R.id.action_save) {
            ...
            try {
                if (object.getString("title").contains("Stock Issued") || object.getString("title").contains("Stock Received") || object.getString("title").contains("Stock Loss/Adjustment")) {
                    balancecheck = ((KipJsonFormActivity) getActivity()).checkIfBalanceNegative();
                }
            } catch (Exception e) {
                Log.e(getClass().getName(), e.toString(), e);
            }
